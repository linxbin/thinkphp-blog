...
class Linxb extends TagLib
{
    ...
    protected $tags = array(
        ...
        //文章列表
        'artlist'         => array(
            'attr'  => 'top, hot, cid, aid, orderby', //attr 属性列表,arcid[new|20140413] 指定文档ID
    ...
    public function _artlist($attr, $content)
    {
        ...
        $top        = empty($attr['top']) ? '' : $attr['top'];
        $hot        = empty($attr['hot']) ? '' : $attr['hot'];
...
<?php
    ...
    \$opt['art.status'] = 1;
    \$orderBy = 'art.top desc , art.time desc';
    \$data = D('Article') -> getArticle(\$opt, \$orderBy, \$_pagesize);
    \$_artlist = \$data['list'];
    \$page = \$data['page'];
	foreach(\$_artlist as \$autoindex => \$artlist):
	  
	    \$artlist['url'] = get_content_url(\$artlist['id'], \$artlist['channel_id'], \$artlist['ename']);
?>
str;
        $str .= $content;
        $str .= '<?php endforeach;?>';
...
str;
    ...
    {
        $id = !isset($attr['id']) || $attr['id'] == '' || $attr['id'] == 0 ? -1 : intval($attr['id']); //不能用empty,0,'','0',会认为true
        $str    = <<<str
<?php
	\$_id = $id;
	if(\$_id == -1) {
        \$_navlist = M('Channel')->where('status != -1')->order('sort asc')->select();
    } else {
        \$_navlist = M('Channel')->where(array('id' => \$_id))->find();
    }
...
str;
    ...
    {
        $config = M('config') -> getField('name,value');
        return $config['WEB_DESCRIPTION'];
    }
    public function _beian($attr, $content)
    {
        $config = M('config') -> getField('name,value');
        return $config['WEB_TCP'];
    }
